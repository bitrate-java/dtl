...
* **Real-Time Telemetry Gauges**: Custom SVG radial dials visualizing Temperature (°C), Vibration (mm/s²), and Winding Current (A) directly from virtual or physical sensors.
...
1. **Normal Range**: Temperature ($20\text{--}80^\circ\text{C}$), Vibration ($0.1\text{--}2.5\text{ mm/s}^2$), Current ($5.0\text{--}15.0\text{ A}$).
...
3. **Critical / Failure Thresholds**: Triggered when Temperature $> 100^\circ\text{C}$, Vibration $> 4.0\text{ mm/s}^2$, or Current $> 20\text{ A}$. The UI flashes alert beacons and recommends an emergency shutdown.
